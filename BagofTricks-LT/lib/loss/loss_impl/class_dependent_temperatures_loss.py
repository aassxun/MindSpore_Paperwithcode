...
class CDT(CrossEntropy):
    ...
    def __init__(self, para_dict=None):
        ...
        self.cdt_weight = ms.Tensor([(max(self.num_class_list) / i) ** self.gamma for i in self.num_class_list])
    ...
    def update(self, epoch):
        ...
        if not self.drw:
            ...
            self.weight_list = msnp.ones(self.cdt_weight.shape)
